...
function DialogFooter({
  ...
  return (
    <div
      ...
      className={cn(
        "-mx-4 -mb-4 flex flex-col-reverse items-end gap-2 rounded-b-xl border-t bg-muted/50 p-4 sm:flex-row sm:items-center sm:justify-end",
        className,
      )}
      {...props}
    >
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          /* Fade-only enter/exit — zoom-in-95 scales the layer and tends to blur text on HiDPI / fractional layouts */
          "data-open:fade-in-0 data-closed:fade-out-0 fixed top-1/2 left-1/2 z-50 grid min-w-0 w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl bg-background p-4 text-sm outline-none ring-1 ring-foreground/10 duration-100 data-closed:animate-out data-open:animate-in sm:max-w-sm",
          className,
        )}
        {...props}
