...
export default function ReminderEditForm({
  ...
  const mutation = useMutation({
    mutationFn: async (data: ReminderFormValues) => {
      if (reminder) {
        return updateReminder(reminder.id, toReminderUpdate({ ...data, user_id: user?.id } as Database["public"]["Tables"]["reminders"]["Update"]), createClient());
      }
      // create
      const supabase = createClient();
      const { data: newData, error } = await supabase.from("reminders").insert(toReminderInsert({ ...data, user_id: user?.id } as Database["public"]["Tables"]["reminders"]["Insert"])).select().single();
