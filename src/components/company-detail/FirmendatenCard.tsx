...
import { safeString } from "@/lib/supabase/utils";
...
export default function FirmendatenCard({ company }: Props) {
  ...
  const formatWebsite = (website: string | null) => {
    if (!website) return safeString(website);
  ...
  const formatTelefon = (telefon: string | null) => {
    if (!telefon) return safeString(telefon);
    return (
  ...
  const formatEmail = (email: string | null) => {
    if (!email) return safeString(email);
    return (
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-sm text-gray-900">{safeString(company.firmenname)}</p>
            </div>
            <div>
              <div className="text-sm font-medium text-gray-700">Rechtsform</div>
              <p className="text-sm text-gray-900">{safeString(company.rechtsform)}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-sm text-gray-900">{safeString(company.firmentyp)}</p>
            </div>
            <div>
