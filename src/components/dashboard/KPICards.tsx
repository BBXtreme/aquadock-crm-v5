  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      {kpis.map((kpi, index) => {
        ...
        const trendColor = kpi.trendColor || (isPositive ? '#24BACC' : 'rgb(244 63 94)') // marine for positive, rose for negative
        const arrowColor = isPositive ? 'text-[#24BACC]' : 'text-rose-500'

        return (
          <Card key={index} className="border border-border bg-card text-card-foreground shadow-sm rounded-xl">
