...
export async function getCompanies(client = createServerSupabaseClient()): Promise<Company[]> {
  const { data, error } = await client.from('companies').select('*');
  if (error) throw new Error(`Failed to fetch companies: ${error.message}`);
  return data ?? [];
}

export async function getCompanyById(id: string, client = createServerSupabaseClient()): Promise<Company | null> {
...
export async function createCompany(company: CompanyInsert, client = createServerSupabaseClient()): Promise<Company> {
  const { data, error } = await client.from('companies').insert(company).select().single();
  if (error) throw new Error(`Failed to create company: ${error.message}`);
  return data;
}

export async function updateCompany(id: string, updates: CompanyUpdate, client = createServerSupabaseClient()): Promise<Company> {
  const { data, error } = await client.from('companies').update(updates).eq('id', id).select().single();
  if (error) throw new Error(`Failed to update company: ${error.message}`);
  return data;
}

export async function deleteCompany(id: string, client = createServerSupabaseClient()): Promise<void> {
