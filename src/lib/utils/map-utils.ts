...
// deduplication
// The `getStatusIcon` function generates a Leaflet icon based on the
// status of a company, using predefined colors
// The `getOsmPoiIcon` function generates a simple Leaflet icon for
// OSM POIs, with styling that adapts to dark mode
// The code is designed to be robust and handle various edge cases,
// such as API rate limits and timeouts, while providing informative
// console logs for debugging purposes
// The `fetchOsmPois` function includes a debounce mechanism to prevent
// excessive API calls when the map is being moved or zoomed rapidly,
// ensuring better performance and user experience.
// The function also implements an exponential backoff strategy for
// retries in case of rate limiting, and it attempts multiple Overpass
// API endpoints to increase the chances of a successful response.
// The icons generated by `getStatusIcon` and `getOsmPoiIcon` are
// designed to be visually distinct and informative, using colors
// and symbols to convey information at a glance on the map.
// The types defined in this file, such as `PoiCategory` and `OsmPoi`,
// help ensure type safety and clarity when working with POI data
// and category configurations.
// The utility functions in this file can be easily imported and used
// in various components of the app that require map functionality,
// such as displaying company locations, showing nearby amenities,
// or visualizing customer statuses on a map interface.
// The code is structured to allow for easy extension in the future,
// such as adding more complex icons, supporting additional POI attributes, or integrating with other mapping libraries if needed.
