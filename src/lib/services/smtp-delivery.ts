/**
 * Nodemailer + system SMTP resolution. No `createServerSupabaseClient` import so
 * this module is safe to load from server utilities imported by non-server graphs (e.g. tests).
 */
...
/**
 * SMTP for server-triggered notifications: prefer the acting admin's
...
/**
 * Send HTML (and optional plain text) using {@link getSystemSmtpConfigForNotifications}.
 * When `text` is set, both parts are set for better client support.
