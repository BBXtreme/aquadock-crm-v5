...
/** Runs before React / Supabase; recovery hash can be consumed by createBrowserClient before login reads it. */
const pwRecoveryBootstrapScript = `(function(){try{var path=location.pathname||"";if(!/\\/login\\/?$/.test(path))return;var frag=(location.hash||"").replace(/^#/,"");if(frag.indexOf("type=recovery")>=0||frag.indexOf("type%3Drecovery")>=0)sessionStorage.setItem(${JSON.stringify(PW_RECOVERY_SESSION_STORAGE_KEY)},"1");}catch(_){}})();`;
