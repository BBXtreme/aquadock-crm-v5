...
import {
	createTimelineEntry,
	getAllTimelineForUser,
} from "@/lib/supabase/services/timeline-server";

// GET /api/timeline
// Returns timeline entries for current authenticated user
export async function GET(_request: NextRequest) {
	try {
		const timeline = await getAllTimelineForUser(
			"dev-mock-user-11111111-2222-3333-4444-555555555555",
		);
...
export async function POST(request: NextRequest) {
	...
	try {
		try {
			...
			console.log(
				"[POST /api/timeline] Raw received body:",
		...
		console.log(
			"[POST /api/timeline] Final payload to service:",
		...
		console.log(
			"[POST /api/timeline] Success – created entry:",
			timelineEntry.id,
		);
